...
path1 = "../../Music/Musique/Mp3_files/Caravan/*.*"
path2 = "overlayedDrums/*.*"
...
def massOverlayDrum(path) :
    ...
    for file in glob.glob(path) :
        original_sound = AudioSegment.from_mp3(file)
        if (count % 2 == 0 ) :
        ...
        overlayed_file_name = "overlayedDrums/%s_overlay%s.mp3" %(raw_file_name.split(".mp3",1)[0], backing_name)
...
def massOverlayBass(path) :
    ...
    for file in glob.glob(path) :
        ...
        # nommer le fichier fils
        raw_file_name = file.split("\\",1)[1]
        overlayed_file_name = "overlayedDrumsAndBass/%s_overlay%s.mp3" %(raw_file_name.split(".mp3",1)[0], backing_name)
        # créer le fichier fils
        overlaying(original_sound, background, 0, overlayed_file_name)
        # print(overlayed_file_name)
        count += 1
